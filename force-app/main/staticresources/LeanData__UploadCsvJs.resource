...
function upload(operation, objectType){
	...
	j$('#uploadFileText').text('File Uploading');
	j$('#uploadFileMessage').css('color','green');
	j$('#uploadFileMessage').text('Upload in progress...');
				...
				if(operation == 'territoryGeo'){
					...
					if(result.startsWith('Errors Occurred')){
						...
						var htmlString = '<table class="upload-file-errors"><tbody>'
							...
							+ '</td></tr><tr><td>Number of rows with City populated but missing Country (or State/Country for US): </td><td>' + resultArray[2]
							...
							+ ((resultArray.length == 7 || resultArray.length == 8) ? ('</td></tr><tr><td style="width:500px">Duplicate Row Pairs: </td><td style="max-width:100px">' + resultArray[6]) : ('</td></tr>'))
							+ (resultArray.length == 8 ? ('</td></tr><tr><td style="width:500px">Invalid Owners: </td><td>' + resultArray[7]) : ('</td></tr>'))
							+ '</td></tr></tbody></table>';
						j$('#uploadFileMessage').html(htmlString);

					} else {
						j$('#uploadFileText').text('Upload CSV Success');
						j$('#uploadFileMessage').text(result);
						if(result == 'Draft Territories successfully created.'){
							var draftSegmentName = params.segment + ' - Draft';
						} else if(result.includes('Invalid Column Headers')){
							j$('#uploadFileMessage').css('color','red');
...
function sendProcessLeadPacket(rows, count, operation, key, pairList, sendNotification, headerArr, objectType) {
	...
	var packet = {
		key: operation + '_' + key + '_PACKET_' + count,
		rows: JSON.stringify(LTSs),
	};
	Visualforce.remoting.timeout = 120000;
	console.log(packet);
	Visualforce.remoting.Manager.invokeAction(
		namespace + 'RemotingRouter.receiveListAnalyzerPacket', packet,
		function(result, event) {
			if (result=='PACKET RECEIVED') {
				if (rows.length==0) {
