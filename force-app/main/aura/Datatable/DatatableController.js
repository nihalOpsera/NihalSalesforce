({
	doInit : function(component, event, helper) {
        ...
        component.set("v.columnHeight", columns && columns.filter(cur => cur.type === 'button').length == 0 ? (
            columns && columns.filter(cur => cur.type === 'action').length == 0 ? '1.75rem' : '1.875rem'
        ) : (helper.isPortal() ? '3.05rem' : '2.5625rem'));
    ...
    setTableHeight : function (component, event, helper) {
        ...
        else component.set("v.tableHeight", 'calc(100% - ' + (component.get('v.hideControls') ? '67px' : '102px') + '); position: relative; top: -2px');    
    },
