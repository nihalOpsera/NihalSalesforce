({
    validateGate : function(component,event,helper) {
        ...
        if(oppty.Business_Type__c !== 'Standard Opportunity' && oppty.Business_Type__c !== 'Committed Upsell'){
            return;
        }
        ...
        if(oppty.StageName === 'Create Offer (Bid)' && oppty.NF_LoA_Bypass__c === 0 ){
            if(oppty.Phase_Status__c == 'Offer in Progress' && !oppty.Gate_3_Bypass_no_offer_support__c){
        ...
        if(oppty.StageName === 'Win the Case (Negotiate)' && oppty.Blanket_Approval_Condition__c === null){
    ...
    handleSaveOfferRecord: function(component,event,helper) {
        component.find("recordLoaderOffer").saveRecord($A.getCallback(function(saveResult) {
            if (saveResult.state === "SUCCESS" ) {
                console.log('offer updated with sucess');
            }
        }));
    },
