({    
    ...
    Generate : function(component, event, helper) { 	
        var filePath;	
        var recId = component.get("v.recordId");	
        filePath = '/one/one.app#/alohaRedirect/apex/Apttus_Proposal__ProposalGenerate?id='+recId;	
        var url = location.href;	
        var baseURL = url.substring(0, url.indexOf('/', 14));        	
        console.log('>>>'+baseURL+filePath);        	
        var link=baseURL+filePath;	
    ...
    acceptProposal : function(component, event, helper) {        
        ...
        action.setCallback(this,function(response){
            if(response.getState()==="SUCCESS"){
                ...
                if(type != "Indirect CPQ" || (type == "Indirect CPQ" && is_rg_nam != "True")){
    ...
    submitforApprovalEBG : function(component, event, helper) {        
        var filePath;
        var recId = component.get("v.recordId");
        filePath = '/apex/Apttus_Approval__PreviewSubmitApprovals?id='+recId+'&hideSubmitWithAttachments='+true;
        var url = location.href;
        var baseURL = url.substring(0, url.indexOf('/', 14));        
        console.log('>>>'+baseURL+filePath);        
        var link=baseURL+filePath;
            ...
            if(response.getState()==="SUCCESS"){
                ...
                else
                {
                    ...
                    toastEvent1.setParams({
                        //title : res,
                        message: 'There was an error sending the data to NPT Cloud. Please contact your support.',
                        duration:' 5000',
                        key: 'info_alt',
                        type: 'error',
                        mode: 'pester'
