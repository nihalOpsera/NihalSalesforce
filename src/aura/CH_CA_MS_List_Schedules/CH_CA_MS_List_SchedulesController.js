    ...
    exportSchedule: function(component, event, helper) {
        ...
        .then(function(result){
            if(!component.get("v.isRecordsExist")){
                ...
                component.set("v.Spinner", false);
                if(component.get("v.radioValue")=="TimeSlot"){
                    helper.showToast("ERROR","","The selected Schedule(s) do(es)n't have Timeslot defined. There is no data to be exported.");
                }  
                else{
                    helper.showToast("ERROR","","The selected Schedule(s) do(es)n't have Timeslot Members defined. There is no data to be exported.");
                }
            }
        });        
    },
