({  
    ...
    displayMessageButton : function(component,event,helper) {
        if(component.get("v.LabelList").length==1 ){                
            ...
            if (component.get("v.LabelList")[0] == 'HWS_WCheck_UnKnowns' || component.get("v.LabelList")[0] == 'HWS_PO_MISSING') {
    ...
    updateParentCaseStatusBlank: function(component,event,helper){
        var promise = new Promise( function( resolve , reject ) {
            const sharedjs = component.find("sharedJavaScript");
            resolve(sharedjs.apex(component, 'updateParentCaseStatusBlank',{childCaseId : component.get("v.recordId")}));
        });           
        return promise; 
    },
    //This method used to initialize all attribute value 
    ...
    cloneAllCases: function(component, event, helper) {
        ...
        action.setCallback(this, $A.getCallback(function (response) {
            var state = response.getState();
            if (state === "SUCCESS") {
                // var result= JSON.parse(response.getReturnValue())//comment for blocker issue
                component.set("v.isSpinner", false);  
                var toastEvent = $A.get("e.force:showToast");
                toastEvent.setParams({
                    title : 'Success',
                    ...
                    messageTemplate: 'NOTICE: This case has been split to escalate the unknown warranty items and/or quotation required items to a Nokia specialist.Known Warranty items: existing case {0}.Unknown items: new case {1}.',
                    messageTemplateData: [
                        {
                            url: '/'+parentCaseId,
							label: 'here',
                             // label: component.get("v.existingCaseNumber"),//commented due to blocker issue
                        },
                        {
                              url: '/'+response.getReturnValue(),
                              label: 'here',
							//  url: '/'+result.Id,//commented due to blocker issue
                           // label: result.CaseNumber,//commented due to blocker issue
                        }
                    ]
                });
                toastEvent.fire();
