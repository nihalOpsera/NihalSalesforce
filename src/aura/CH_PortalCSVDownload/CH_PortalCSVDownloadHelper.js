({
    ...
    convertArrayOfObjectsToCSV : function(component,objectRecords,Recordtype){
        ...
        if(Recordtype=="SWS"){
            keys = ['CaseNumber','Severity__c','Contact.Name','HWS_Customer_Reference_Number__c','Subject','Status','CH_Product_Name__c','CH_ProductVariant__c','CH_Product_Release__c','CH_Solution_Name__c','CH_SW_Release_Name__c','CH_SW_Build_Name__c','CH_NetworkElementAsset__r.Name','Account.Name','Entitlement.Servicecontract.Name','Country__c','Contact.Account.Name','CH_ReportedDate__c','CH_InitialResponse__c','CH_SystemRestored__c','CH_TemporarySolutionProvided__c','CH_SolutionProvided__c','ClosedDate','CH_LastCustomerInteraction__c','CH_Summary__c','CH_InvitationLink__c'];
            csvStringResult = '';
            csvStringResult +=  ['Support Ticket Number','Severity','Contact Name','Customer Reference Number','Subject','Status','Product','Product Variant','Product Release','Solution','SW Release','SW Build','Network Element Asset.Name','Account.Name','Service Contract Number','Country','Contact.Account.Name','Reported Date','Initial Response Date','Restored Date','Temporary Solution Provided Date','Solution Provided Date','Date/Time Closed','Last Customer Interaction','Summary to Customer','Survey'];
												...
												if(JSON.stringify(objectRecords[i].Parent) != undefined && JSON.stringify(objectRecords[i].Parent.Street_Address_2__c) !== null && JSON.stringify(objectRecords[i].Parent.Street_Address_2__c) != undefined){
													...
													if(JSON.stringify(objectRecords[i].Parent) != undefined && JSON.stringify(objectRecords[i].Parent.Street_Address_3__c) !== null && JSON.stringify(objectRecords[i].Parent.Street_Address_3__c) != undefined){
                                                        csvStringResult += JSON.stringify(objectRecords[i].Parent.Street_Address_3__c);
                                                         ...
                                                         //NOKIASC-35553 | Start
                                                        else if(skey === 'CH_Summary__c'){
                        									if(JSON.stringify(objectRecords[i].CH_Summary__c) != undefined && JSON.stringify(objectRecords[i].Country__c) !== null){
                                                                var sSummary = JSON.stringify(objectRecords[i].CH_Summary__c);
                                                                sSummary = sSummary.replace(/\n/g, " ");
                                                                csvStringResult += sSummary;
                                                            }  
                                                        }
                                                        //NOKIASC-35553 | End
