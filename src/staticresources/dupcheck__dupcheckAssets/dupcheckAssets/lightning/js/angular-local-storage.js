...
(function (window, angular) {
  ...
  angular.module('LocalStorageModule', []).provider('localStorageService', function () {
    ...
    this.$get = [
      ...
      '$parse',
      '$timeout',
      function ($rootScope, $window, $document, $parse, $timeout) {
        var self = this;
        ...
        var checkSupport = function () {
          try {
            ...
            if (self.defaultToCookie) storageType = 'cookie';
        ...
        var clearAllFromLocalStorage = function (regularExpression, type) {
          ...
          try {
            ...
            if (!browserSupportsLocalStorage && !self.defaultToCookie) return false;
            var prefixLength = prefix.length;
        ...
        var addToCookies = function (key, value, daysToExpiry, secure) {
          ...
          try {
            ...
            if (!!key) {
              ...
              $document.cookie =
                deriveQualifiedKey(key) + '=' + encodeURIComponent(value) + expiry + cookiePath + cookieDomain;
            }
          } catch (e) {
            $rootScope.$broadcast('LocalStorageModule.notification.error', e.message);
            return false;
        ...
        var getFromCookies = function (key) {
          ...
          for (var i = 0; i < cookies.length; i++) {
            ...
            if (thisCookie.indexOf(deriveQualifiedKey(key) + '=') === 0) {
              var storedValues = decodeURIComponent(
                thisCookie.substring(prefix.length + key.length + 1, thisCookie.length)
              );
              try {
